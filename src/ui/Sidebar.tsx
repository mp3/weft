...
export function Sidebar({ parsed, onToggle, open, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed right-0 top-0 z-50 flex h-full w-80 shrink-0 flex-col border-l border-zinc-200 bg-white transition-transform duration-200 ease-in-out md:static md:z-auto md:translate-x-0 md:transition-none ${
          ...
          {TABS.map((tab) => (
            ...
            >
              ...
              {tab.id === 'tasks' && openTaskList.length > 0 && (
                <span className="ml-1 text-xs text-zinc-400">({openTaskList.length})</span>
              )}
            </button>
          ))}
        </nav>
